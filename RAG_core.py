...
def build_bm25_index(lc_docs):
    corpus = [_tokenize(d.page_content) for d in lc_docs]
    return BM25Okapi(corpus)

def bm25_retrieve(bm25, lc_docs, query, k=4):
    scores = bm25.get_scores(_tokenize(query))
    top_idx = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:k]
    return [lc_docs[i] for i in top_idx]

def rrf_fuse(vec_docs, kw_docs, k=60, top_n=6):
    ranks = {}

    def add(docs):
        for r, d in enumerate(docs):
            key = (d.metadata.get("start",0), d.page_content[:80])
            ranks[key] = ranks.get(key, 0) + 1/(k+r+1)
    ...
    pool = vec_docs + [d for d in kw_docs if d not in vec_docs]

    fused_docs = []
    for key,_ in ranked[:top_n]:
        for d in pool:
            if (d.metadata.get("start",0), d.page_content[:80]) == key:
                fused_docs.append(d)
                break
...
# QA
# ======================
def answer_question(video_id, question, retriever, history, lc_docs=None, bm25=None):
    llm = get_llm()
    ...
    # 3) Retrieve + answer
    vec_docs = retriever.invoke(rewritten)
    kw_docs = []
    if bm25 and lc_docs:
        kw_docs = bm25_retrieve(bm25, lc_docs, rewritten, k=4)
    
    docs = rrf_fuse(vec_docs, kw_docs)
